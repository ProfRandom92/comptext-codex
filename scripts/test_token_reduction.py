...
class TokenReductionTest:
    ...
    def run_tests(self) -> Dict:
        ...
        avg_reduction = sum(ex['reduction'] for ex in self.examples) / len(self.examples)
        
        return {
            'examples': self.examples,
            'total_nl_tokens': total_nl_tokens,
            'total_ct_tokens': total_ct_tokens,
            'total_reduction': ((total_nl_tokens - total_ct_tokens) / total_nl_tokens) * 100,
    ...
    def generate_markdown_report(self, results: Dict, output_file: str = "TOKEN_REDUCTION_RESULTS.md"):
        """
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write("by comparing natural language prompts with CompText DSL commands.\n\n")
            f.write("**Test Configuration:**\n")
            f.write("- Tokenizer: GPT-4 approximation (character-based: ~3.5 chars/token for NL, ~4 chars/token for DSL)\n")
            f.write(f"- Number of test cases: {len(results['examples'])}\n")
            f.write("- Methodology: Based on OpenAI's guidance (1 token ≈ 4 chars for English text)\n\n")
            f.write("---\n\n")
            ...
            f.write("|--------|-------|\n")
            f.write(f"| Total Natural Language Tokens | {results['total_nl_tokens']} |\n")
            f.write(f"| Total CompText Tokens | {results['total_ct_tokens']} |\n")
            f.write(f"| **Total Token Reduction** | **{results['total_reduction']:.1f}%** |\n")
            f.write(f"| Average Token Reduction | {results['avg_reduction']:.1f}% |\n")
            f.write(f"| Tokens Saved | {results['total_nl_tokens'] - results['total_ct_tokens']} |\n\n")
            ...
            f.write(f"- **Average Reduction:** {results['avg_reduction']:.1f}% across all examples\n")
            f.write(f"- **Total Tokens Saved:** {results['total_nl_tokens'] - results['total_ct_tokens']} ")
